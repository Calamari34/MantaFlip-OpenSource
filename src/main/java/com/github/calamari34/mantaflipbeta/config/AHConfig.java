...
public class AHConfig extends Config {
    public AHConfig() {
        super(new Mod("Lilase", ModType.SKYBLOCK, "/assets/MantaFlip.jpg", 84, 84), "MantaFlip.json");
        initialize();
        addListener("WEBHOOK", () -> ConfigHandler.setString("Webhook", WEBHOOK));
        addListener("REMOTE_CONTROL", () -> ConfigHandler.setBoolean("RemoteControl", REMOTE_CONTROL));
        addListener("BOT_TOKEN", () -> ConfigHandler.setString("BotToken", BOT_TOKEN));
        addListener("LOG_CHANNEL", () -> ConfigHandler.setString("LogChannel", LOG_CHANNEL));
